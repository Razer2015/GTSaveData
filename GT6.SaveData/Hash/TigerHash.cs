...
namespace GT.SaveData.Hash
{
    public abstract class BlockHashAlgorithm : HashAlgorithm
    {
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            ...
            if (BufferFill > 0)
            {
                ...
                else
                {
                    ...
                    ProcessBlock(ba_PartialBlockBuffer, 0, 1);
                    l_TotalBytesProcessed += BlockSize;
                    i_PartialBlockFill = 0;
                    ibStart += i;
                    cbSize -= i;
                }
        ...
        internal static class BitTools
        {
            ...
            public static void TypeBlindCopy(byte[] sourceArray, int sourceIndex,
                uint[] destinationArray, int destinationIndex, int sourceLength)
            {
                ...
                int iCtr;
                for (iCtr = 0; iCtr < sourceLength; iCtr += 4, sourceIndex += 4, ++destinationIndex)
                    destinationArray[destinationIndex] = BitConverter.ToUInt32(sourceArray, sourceIndex);
            }

            public static void TypeBlindCopy(uint[] sourceArray, int sourceIndex,
                byte[] destinationArray, int destinationIndex, int sourceLength)
            {
                ...
                int iCtr;
                for (iCtr = 0; iCtr < sourceLength; ++iCtr, ++sourceIndex, destinationIndex += 4)
                ...
                UInt64[] destinationArray, int destinationIndex, int sourceLength)
            {
                ...
                int iCtr;
                for (iCtr = 0; iCtr < sourceLength; iCtr += 8, sourceIndex += 8, ++destinationIndex)
                ...
                byte[] destinationArray, int destinationIndex, int sourceLength)
            {
                ...
                int iCtr;
                for (iCtr = 0; iCtr < sourceLength; ++iCtr, ++sourceIndex, destinationIndex += 8)
                    Array.Copy(BitConverter.GetBytes(sourceArray[sourceIndex]),
    ...
    public partial class Tiger : BlockHashAlgorithm
    {
        ...
        protected override byte[] ProcessFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            var paddingSize = (int)(((ulong)inputCount + (ulong)l_TotalBytesProcessed) % (ulong)BlockSize);
